(function() {
	...
	angular.module('angular-bugsnag', ['async-script']).provider('Bugsnag',
		function() {
			...
			this.$get = ['asyncScript', '$window',
				function(asyncScript, $window) {
					if (!enabled) return $bugsnag;

					asyncScript.load(scriptUrl).then(function() {
						if (!$window.Bugsnag) {
							return;
						}
	...
	/**
	 * Decorate the $exceptioHandler service to notify Bugsnag about exceptions
	 */
